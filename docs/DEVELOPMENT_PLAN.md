...
`docs/PRD.md`, `docs/TSD.md`, 원본 포트폴리오 자료에서 공개 가능한 Evidence 사본을 기반으로, 기업 인턴 지원에 바로 사용할 수 있는 송채우 포트폴리오 웹사이트를 운영한다.
...
- 공개 Evidence 사본: `public/evidence/`
...
사이트는 Vite + React + TypeScript 기반 정적 SPA로 구현한다.
...
- Contact: 이메일 CTA와 함께 보면 좋은 공개 자료 링크
...
Evidence 링크는 배포 방문자가 로그인 없이 접근할 수 있는 내부 정적 경로를 우선한다. 공개할 수 없는 자료는 `private`으로 유지하고 사이트에 직접 렌더링하지 않는다.
...
5. 공개 Evidence 자산을 `public/evidence/`에 배치하고 registry를 내부 경로로 연결
6. 로컬 Vite 서버 실행 및 브라우저 QA
7. 확인 필요 항목은 확정 표현으로 바꾸지 않고, 공개 사본/후보 기여 문장으로 분리
