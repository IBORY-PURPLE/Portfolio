...
현재 구현은 Vite + React + TypeScript 기반 정적 SPA다. `docs/PRD.md`를 기준으로, 기획/세일즈/고객 인터뷰 같은 정성적 역량을 앞에 세우고 기술 실행력을 뒤에서 받쳐주는 포트폴리오 웹사이트를 제공한다.
...
- 공개 가능한 근거 자료는 `public/evidence/` 정적 자산과 `src/data.ts` registry로 관리
...
- Build: Vite
- UI: React, TypeScript
- Routing: `src/App.tsx` 내부 hash routing
- Styling: 전역 `styles.css`와 CSS custom properties
- State: 로컬 상태 중심, 필요 시 URL query로 필터 상태 관리
- Data: `src/data.ts`
- Charts/diagrams: 프로젝트 상세에 필요한 경우 Mermaid 또는 가벼운 SVG/HTML 구조
- Deployment: Vercel

프로젝트가 작고 포트폴리오의 정교한 톤이 중요하므로 단일 전역 CSS 체계로 유지한다. 새 스타일 체계를 추가하기보다 기존 디자인 토큰과 컴포넌트 클래스를 확장한다.
...
|-- public/
|   |-- assets/
|   |   |-- portfolio-hero-minimal.png
|   |   |-- portfolio-hero-dark.png
|   |   `-- projects/
|   `-- evidence/
|-- src/
|   |-- App.tsx
...
- `Header`: 로고/이름, 주요 섹션 링크, 다크/라이트 토글
- `Footer`: 이메일, 검증 가능한 공개 사본 기준의 업데이트 정보
...
- `EvidenceList`: 프로젝트 상세에서는 Evidence browser, Credentials에서는 compact disclosure로 렌더링
- `MarkdownPreview`: H1/H2/H3, 코드 블록 토큰 강조, 콜아웃, 체크리스트, 구분선, 문서 정보 토글 렌더링
- `PdfPreview`: toolbar hidden iframe, 페이지 이동, 확대/축소, loader, 원본 파일 열기와 다운로드 fallback 제공
...
런타임에서는 외부 API나 Google Drive를 호출하지 않는다. 공개 가능한 근거 자료는 `public/evidence/` 정적 파일로 내재화하고, 앱은 `/evidence/<file>` 경로만 참조한다.

권장 흐름:

1. 원본 자료에서 공개 가능한 Markdown/PDF/이미지와 비공개 자료를 구분
2. 공개 사본을 `public/evidence/` 또는 `public/assets/`로 배치
3. `src/data.ts`의 evidence registry에 내부 정적 경로 등록
4. `scripts/sync-evidence-assets.ps1 -VerifyOnly`로 파일 존재와 PDF page count 확인
5. 개인정보, 토큰, 내부 계정 정보는 제외하거나 placeholder로 치환
...
- 런타임 Evidence URL은 내부 정적 경로를 우선하며, 비공개 자료는 직접 렌더링하지 않음
